...
class Teacher(Person):
    def teacher_attributes(self, salary=''):
        self.salary = salary

    def reference(self):
        return f'Object name: {self.first_name}, surname: {self.last_name}, gender: {self.gender}, age: {self.age}, ' \
               f'salary: {self.salary}.'

class Student(Person):
    def student_attributes(self, scholarship=''):
        self.scholarship = scholarship

    def reference(self):
        return f'Object name: {self.first_name}, surname: {self.last_name}, gender: {self.gender}, age: {self.age}, ' \
               f'subject: {self.scholarship}.'

t = Teacher(first_name = 'Carol', last_name = 'Johnson', gender = 'female', age = '28')
t.teacher_attributes('$2500')
print(t.reference())
s = Student(first_name = 'Carol', last_name = 'Johnson', gender = 'female', age = '28')
s.student_attributes('$1500')
print(s.reference())
...
class Mathematician:
    ...
    def filter_leaps(self, args):
        return [a for a in args if a % 4 == 0 or (a % 100 == 0 and a % 400 == 0)]
    ...
    def save(self):
        if os.path.exists('logs.txt'):
            with open('logs.txt', 'a') as file:
                file.write(f'Error msg: {self.message} \n')
        else:
            with open('logs.txt', 'w') as file:
                file.write(f'Error msg: {self.message} \n')
a = CustomException(KeyError)
a.save()